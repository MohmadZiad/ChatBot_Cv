...
function parseRequirements(text: string): JobRequirement[] {
  // صيغة مرنة: كل سطر Requirement اختياريًا فيه must/weight
  // أمثلة:
  // React, must, 2
  // TypeScript, 1
  // Tailwind
...
export default function AIConsole() {
      ...
      content: (
        <div>
          ...
          <ul className="text-xs opacity-70 mt-2 list-disc ps-5">
            ...
            <li>3) اضغط {tt("chat.run")} — أرجع لك النتيجة داخل المحادثة.</li>
          </ul>
        </div>
      ),
    },
  ...
  // تشغيل التحليل الكامل: create job → upload cv → run analysis
  const run = async () => {
    if (!cvFile || reqs.length === 0) {
      push({
        role: "bot",
        content: (
          <div className="text-sm">رجاءً أدخل المتطلبات وارفع CV أولًا.</div>
        ),
      });
      return;
    }
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="relative rounded-[28px] border border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5 p-4 shadow-xl overflow-hidden">
        ...
        <div className="space-y-2 max-h-[55vh] overflow-y-auto pe-1">
          ...
          {/* لو في نتيجة نهائية نعرض بطاقة مستقلة أيضًا (اختياري) */}
          {result && (
        ...
        {/* حواكم الإدخال/التحكم تحت الرسائل */}
        <div className="mt-3 grid gap-2">
          {/* عنوان ووصف (اختياري) */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <input
              placeholder="Job Title (اختياري)"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              className="rounded-xl border px-3 py-2 bg-white/90 dark:bg-white/10"
            />
            <input
              placeholder="Job Description (اختياري)"
              value={description}
            ...
            <div className="mt-2 flex items-center justify-between">
              <label className="inline-flex items-center gap-2 text-sm">
                ...
                <span className="opacity-80">
                  {cvFile ? cvFile.name : "أرفق CV (PDF/DOCX)"}
                </span>
              </label>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {loading ? "جاري العمل…" : tt("chat.run")}
                </button>
              </div>
            </div>
          </div>
